    ...
    ui(new Ui::MainWindow)
{
    ...
    */
     ...
     *  init network
    */
    this->net_socket = new NetClientThread( ui->lineEdit_server_ip->text(), ui->lineEdit_port_num->text().toUInt() );
    ...
    connect( (QObject*)this->net_socket, SIGNAL(net_data_plot(quint64*,quint32) ),this,SLOT(on_net_plot_read(quint64*,quint32)));
    ...
    connect( (QObject*)this->net_socket, SIGNAL(net_data_plot(float*, quint32)),this,SLOT(on_net_plot_read(float*, quint32)));
    /*
     * ui state.
     **/
    qDebug() << "ui init!@";
...
void MainWindow::init_qwt()
{
    ...
    (void) new QwtPlotMagnifier(ui->qwt_ch->canvas());
    (void) new QwtPlotPanner(ui->qwt_ch->canvas());
...
void MainWindow::on_pushButton_set_clicked()
{
    ...
    ret = net_socket->set_connect( ui->lineEdit_server_ip->text(), ui->lineEdit_port_num->text().toUInt());
    if( !ret ) {
        qDebug("socket Connection failed!!") ;
        qDebug() << "to ip :"  << ui->lineEdit_server_ip->text();
        qDebug() << "to port :"  << ui->lineEdit_port_num->text();
...
void MainWindow::on_net_plot_read(quint64 *block, quint32 length)
{
    qint64 channel_a[500];
    qint64 channel_b[500];
    qint64 channel_c[500];
    qint64 channel_d[500];
    union k{
        quint64 u;
        float f;
    } channel_data[500][4];
...
#endif
        channel_data[i][0].u = block[i];
        channel_data[i][1].u = block[i + 500];
        channel_data[i][2].u = block[i + 1000];
        channel_data[i][3].u = block[i + 1500];
    }
    for (quint32 i = 0; i < 500; i ++) {
        channel_a_d[i] = (double)channel_data[i][0].f ;
        channel_b_d[i] = (double )channel_data[i][1].f ;
        channel_c_d[i] = (double )channel_data[i][2].f ;
        channel_d_d[i] = (double )channel_data[i][3].f ;
...
void MainWindow::on_net_plot_read(float* f_data, quint32 length)
{

    double channel_a_d[2048];
    double channel_b_d[2048];
    double channel_c_d[2048];
    double channel_d_d[2048];
    qDebug() << "slot plot!";

    for (quint32 i = 0; i < length; i ++) {
        channel_a_d[i] = (double)f_data[i] ;
        //qDebug() << "sample: " << channel_a_d[i];
    }
    if (ui->checkBox_ch1_time->isChecked()) {
        this->qwt_curve1_ch1->attach(ui->qwt_ch);
    ...
    if (ui->checkBox_ch1_fft->isChecked()) {
        this->qwt_curve1_ch1_fft->attach(ui->qwt_fft);
        qwt_plot_fft(CHANNEL_0, channel_a_d, 2048);
    }else{
        this->qwt_curve1_ch1_fft->detach();
    }
...
void MainWindow::on_actionstart_sample_triggered()
{
    on_pushButton_sample_clicked();
}

void MainWindow::on_actionenable_save_triggered()
{
    on_pushButton_close_test_clicked();
}
